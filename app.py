...
def authenticate_gee():
    try:
        if "GEE_JSON_B64" in st.secrets:
            # 1. Limpieza total de la cadena Base64
            b64_str = st.secrets["GEE_JSON_B64"].strip().strip('"').strip("'")
            
            # Fix de padding automático
            ...
            # 3. LIMPIEZA PROFUNDA DE LA LLAVE PRIVADA
            # El JWT Signature falla si los saltos de línea no son físicos (\n)
            pk = json_key["private_key"]
            
            # Quitamos cualquier intento de escape doble o triple que Streamlit haya inyectado
            pk = pk.replace("\\\\n", "\n").replace("\\n", "\n")
            
            # Aseguramos que los encabezados BEGIN y END no tengan espacios laterales
            pk = pk.strip()
            
            json_key["private_key"] = pk

            # 4. Autenticación con el motor oficial
