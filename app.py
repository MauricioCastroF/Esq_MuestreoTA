...
def authenticate_gee():
    try:
        if "GEE_JSON_B64" in st.secrets:
            # 1. Recuperar y limpiar la cadena Base64
            b64_str = st.secrets["GEE_JSON_B64"].strip().strip('"').strip("'")
            
            # Fix automático de padding
            ...
            # 3. RECONSTRUCCIÓN DE LA LLAVE PRIVADA (Fix de Signature)
            pk = json_key["private_key"]
            
            # Limpiamos todos los posibles errores de escape de texto
            # Queremos saltos de línea físicos (ASCII 10), no el texto "\n"
            pk = pk.replace("\\\\n", "\n").replace("\\n", "\n")
            
            # Aseguramos los marcadores de inicio y fin estándar de RSA
            if "-----BEGIN PRIVATE KEY-----" not in pk:
                pk = "-----BEGIN PRIVATE KEY-----\n" + pk + "\n-----END PRIVATE KEY-----"
            
            json_key["private_key"] = pk.strip()

            # 4. Inicialización con el motor moderno de Google
