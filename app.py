...
def authenticate_gee():
    try:
        if "GEE_JSON_B64" in st.secrets:
            # Recuperar y limpiar la cadena de posibles espacios o comillas accidentales
            b64_str = st.secrets["GEE_JSON_B64"].strip().strip('"').strip("'")
            
            # CORRECCIÓN DE PADDING (Múltiplo de 4)
            # Esto evita el error "Invalid base64-encoded string"
            ...
            # Limpieza PEM para la llave privada
