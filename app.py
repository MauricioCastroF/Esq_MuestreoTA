...
def authenticate_gee():
    try:
        if "GEE_JSON_B64" in st.secrets:
            # 1. Recuperar y limpiar profundamente la cadena
            # Eliminamos espacios, saltos de línea y comillas accidentales
            b64_str = st.secrets["GEE_JSON_B64"].strip().replace("\n", "").replace("\r", "").strip('"').strip("'")
            
            # 2. CORRECCIÓN MATEMÁTICA DE PADDING (Múltiplo de 4)
            ...
            # 4. Limpieza de la llave privada (PEM format)
            pk = json_key["private_key"]
            json_key["private_key"] = pk.replace("\\n", "\n").replace("\\\\n", "\n").strip()

            # 5. Autenticación oficial
        ...
        # Mostramos el error exacto para saber si es padding o formato
