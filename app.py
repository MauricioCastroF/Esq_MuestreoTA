...
def authenticate_gee():
    try:
        if "GEE_JSON_B64" in st.secrets:
            ...
            # 2. LIMPIEZA AGRESIVA DE LA LLAVE (Solución al JWT Signature)
            pk = json_key["private_key"]
            
            # Reemplazamos cualquier variante de salto de línea por el real (\n)
            pk = pk.replace("\\n", "\n").replace("\\\\n", "\n")
            
            # Nos aseguramos de que no haya espacios accidentales al inicio o fin
            pk = pk.strip()
            
            # Reasignamos la llave limpia al diccionario
            json_key["private_key"] = pk

            # 3. Autenticación con la librería oficial de Google
