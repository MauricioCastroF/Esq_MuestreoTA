...
def authenticate_gee():
    try:
        if "GEE_JSON" in st.secrets:
            # 1. Recuperamos el texto crudo del secreto
            json_text = st.secrets["GEE_JSON"]
            
            # 2. LIMPIEZA CRÍTICA: Eliminamos dobles escapes que Streamlit suele añadir
            json_text = json_text.replace("\\\\n", "\\n")
            
            # 3. Cargamos el JSON de forma tolerante (strict=False)
            json_key = json.loads(json_text, strict=False)
            
            # 4. Formateamos la clave privada para que GEE la reconozca como un objeto RSA
            if "private_key" in json_key:
                json_key["private_key"] = json_key["private_key"].replace("\\n", "\n")
            
            # 5. Inicializamos con las credenciales refinadas
        ...
        # Error detallado para depuración final
        st.error(f"Error en la estructura de la llave: {e}")
        return False
