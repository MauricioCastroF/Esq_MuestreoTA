...
*Optimizado para pedometría avanzada y decodificación Base64.*
""")

# 2. AUTENTICACIÓN REFORZADA (Base64)
def authenticate_gee():
    try:
        if "GEE_JSON_B64" in st.secrets:
            # Recuperamos la cadena codificada
            b64_str = st.secrets["GEE_JSON_B64"]
            # Decodificamos a JSON puro
            decoded_bytes = base64.b64decode(b64_str)
            json_key = json.loads(decoded_bytes.decode('utf-8'))
            
            # Aseguramos los saltos de línea para el motor RSA
...
# 3. FUNCIONES TÉCNICAS (Lógica Sentinel-2)
    ...
    if uploaded_file is not None and authenticate_gee():
        with st.spinner("Procesando datos satelitales..."):
            ...
            for idx, row in gdf_utm.iterrows():
                ...
                geom_inner = row.geometry.buffer(-15) # Buffer de seguridad
            ...
            if puntos_finales:
                ...
                archivos_zip.append("Navegacion.kml")
                ...
                with open("Resultados.zip", "rb") as f:
                    st.success("✅ ¡Éxito!")
                    st.download_button("📥 Descargar Resultados (.zip)", f, file_name="Muestreo_UY.zip")
    else:
        st.info("👋 Sube tus lotes para empezar.")