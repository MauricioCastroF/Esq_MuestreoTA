...
Optimizado con autenticación reforzada, filtrado IQR y clustering K-Means.
""")

# ==========================================
# 2. AUTENTICACIÓN REFORZADA (Base64 + google.oauth2)
# ==========================================
def authenticate_gee():
    try:
        if "GEE_JSON_B64" in st.secrets:
            ...
            # Usar google.oauth2 para manejar la llave privada de forma segura
...
# ==========================================
# 3. FUNCIONES TÉCNICAS (Satelital y Máscaras)
# ==========================================
    ...
    if uploaded_file is not None and authenticate_gee():
        with st.spinner("Procesando imágenes Sentinel-2..."):
            ...
            for index, row in gdf_utm.iterrows():
                ...
                # Buffer de seguridad (-15m) para evitar bordes
                geom_inner = row.geometry.buffer(-15)
                if geom_inner.is_empty:
                    st.warning(f"⚠️ Lote {lote_id} muy pequeño para el buffer. Saltando.")
                    continue
                ...
                # Colección Sentinel-2 con filtros de calidad
                ...
                # Selección de mejor imagen (Calidad espectral)
                ...
                # Descarga miniatura para reporte
                ...
                # Muestreo IQR (Filtro pedométrico avanzado)
                ...
                if not pixel_data:
                    st.error(f"Falla en extracción de píxeles para {lote_id}")
                    continue
                ...
                # Limpieza de archivos de imagen locales
            ...
            if puntos_finales:
                ...
                kml.save("Puntos_Muestreo.kml")
                archivos_a_comprimir.append("Puntos_Muestreo.kml")
                ...
                with open(zip_name, "rb") as f:
                    st.success("✅ Procesamiento terminado con éxito.")
                    st.download_button("📥 Descargar Resultados (.zip)", f, file_name=zip_name)
    else:
        st.info("👋 Sube tus lotes para iniciar el análisis espectral.")

# ==========================================
# 6. FOOTER
# ==========================================
st.sidebar.markdown("---")
st.sidebar.caption("Optimizado para pedometría y agricultura de precisión.")