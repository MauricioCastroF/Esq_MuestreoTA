...
def authenticate_gee():
    try:
        if "GEE_JSON" in st.secrets:
            ...
            raw_json = st.secrets["GEE_JSON"]
            
            # 2. LIMPIEZA DE "RUIDO": Corregimos los escapes mal interpretados
            # Reemplazamos las dobles barras que Streamlit suele insertar
            clean_json = raw_json.replace("\\\\n", "\\n")
            
            # 3. Cargamos el diccionario ignorando errores de formato menores
            json_key = json.loads(clean_json, strict=False)
            
            # 4. TRATAMIENTO DE LA LLAVE: Forzamos los saltos de línea reales
            if "private_key" in json_key:
                # El motor de Google Earth Engine necesita saltos de línea físicos (\n)
                # no el texto literal "\n". Aquí hacemos esa conversión final.
                json_key["private_key"] = json_key["private_key"].replace("\\n", "\n")
            
            # 5. Inicialización oficial
            ...
            st.error("No se encontró la configuración GEE_JSON.")
            return False
